...
import { ArrowDownRight, ArrowUpRight, BarChart3, CalendarDays, CircleAlert, Clock3, CreditCard, Landmark, PackageSearch, Percent, ShoppingBag, ShoppingCart, TrendingUp } from 'lucide-react';
...
const Analises = () => {
    ...
    const chartData = useMemo(() => (data?.porCanalVenda || []).slice(0, 6).map((row) => ({ nome: row.nome, lucro: row.lucroLiquido })), [data]);
    ...
    return <div className="container" aria-busy={loading}>
        ...
        {loading && !data ? <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">{[0, 1, 2, 3].map((item) => <div key={item} className="card h-36 animate-pulse bg-slate-100 dark:bg-slate-800" />)}</div> : data && <>
            ...
            <div className="grid xl:grid-cols-2 gap-6 mb-6">
                <PerformanceTable title="Canais de venda" description="Compare lucro real com o custo de taxas e frete de cada canal." rows={data.porCanalVenda} type="venda" icon={ShoppingBag} />
                <PerformanceTable title="Canais de compra" description="ROI e giro mostram quais origens de compra retornam melhor." rows={data.porCanalCompra} type="compra" icon={ShoppingCart} />
            </div>

            <div className="grid xl:grid-cols-5 gap-6 mb-6">
                <section className="card p-0 overflow-hidden xl:col-span-3"><div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700"><h2 className="section-heading"><BarChart3 size={19} className="text-green-500" />Lucro por canal de venda</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Use como sinal, não como regra: compare canais com um volume parecido de vendas.</p></div>{chartData.length ? <div className="h-72 p-4"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={{ top: 10, right: 10, left: 4, bottom: 8 }}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke={chartColors.grid} /><XAxis dataKey="nome" tickFormatter={chartLabel} minTickGap={16} tick={{ fill: chartColors.tick, fontSize: 11 }} tickLine={false} axisLine={false} /><YAxis tickFormatter={(value) => `R$${value}`} tick={{ fill: chartColors.tick, fontSize: 11 }} tickLine={false} axisLine={false} width={56} /><Tooltip cursor={{ fill: chartColors.cursor }} formatter={(value) => money(value)} contentStyle={{ backgroundColor: chartColors.tooltipBackground, borderColor: chartColors.tooltipBorder, borderRadius: 12, color: chartColors.tooltipText, boxShadow: isDark ? '0 12px 28px rgba(0, 0, 0, 0.3)' : '0 12px 28px rgba(15, 23, 42, 0.12)' }} labelStyle={{ color: chartColors.tooltipText }} itemStyle={{ color: '#22c55e' }} /><Bar dataKey="lucro" name="Lucro" fill="#22c55e" radius={[8, 8, 0, 0]} /></BarChart></ResponsiveContainer></div> : <EmptyPanel message="Os canais de venda aparecem após a primeira venda completa." />}</section>
                <section className="card p-0 overflow-hidden xl:col-span-2"><div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700"><h2 className="section-heading"><CreditCard size={19} className="text-violet-500" />Origem do produto</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Nacional versus importado.</p></div>{!data.porOrigem.length ? <EmptyPanel message="Sem vendas completas no período." /> : <div className="table-container" aria-label="Deslize a tabela para ver mais colunas"><table><thead><tr><th scope="col">Origem</th><th scope="col">Lucro</th><th scope="col">Margem</th><th scope="col">Giro</th></tr></thead><tbody>{data.porOrigem.map((row) => <tr key={row.nome}><td className="font-medium capitalize">{row.nome}</td><td className="font-mono text-green-600 dark:text-green-400">{money(row.lucroLiquido)}</td><td className="font-mono">{percentage(row.margem)}</td><td className="font-mono">{Math.round(row.giroMedio)} dias</td></tr>)}</tbody></table></div>}</section>
            </div>

            <div className="grid xl:grid-cols-2 gap-6 mb-6">
                <section className="card p-0 overflow-hidden"><div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700"><h2 className="section-heading"><ShoppingCart size={19} className="text-orange-500" />Compra → venda</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Até 12 combinações mais lucrativas; use poucas vendas apenas como sinal inicial.</p></div>{!data.matrizCompraVenda.length ? <EmptyPanel message="A matriz aparece quando houver vendas completas." /> : <div className="table-container" aria-label="Deslize a tabela para ver mais colunas"><table className="min-w-[620px]"><thead><tr><th scope="col">Compra</th><th scope="col">Venda</th><th scope="col">Vendas</th><th scope="col">Lucro</th><th scope="col">Margem</th></tr></thead><tbody>{data.matrizCompraVenda.map((row) => <tr key={`${row.canalCompra}-${row.canalVenda}`}><td>{row.canalCompra}</td><td>{row.canalVenda}</td><td className="font-mono">{number(row.vendas)}{row.vendas < 3 && <span className="block text-xs font-sans text-amber-600 dark:text-amber-400">Amostra pequena</span>}</td><td className="font-mono text-green-600 dark:text-green-400">{money(row.lucroLiquido)}</td><td className="font-mono">{percentage(row.margem)}</td></tr>)}</tbody></table></div>}</section>
                <section className="card p-0 overflow-hidden"><div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700"><h2 className="section-heading"><Clock3 size={19} className="text-amber-500" />Capital parado por compra</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Faixas exclusivas de idade do estoque; priorize o valor, não só a quantidade.</p></div>{!estoque.porCanalCompra.length ? <EmptyPanel message="Não há estoque ativo para analisar." /> : <div className="table-container" aria-label="Deslize a tabela para ver mais colunas"><table className="min-w-[620px]"><thead><tr><th scope="col">Canal</th><th scope="col">Capital</th><th scope="col">30–59 dias</th><th scope="col">60–89 dias</th><th scope="col">90+ dias</th></tr></thead><tbody>{estoque.porCanalCompra.map((row) => <tr key={row.nome}><td className="font-medium">{row.nome}</td><td className="font-mono">{money(row.capital)}</td><td className="font-mono">{number(row.parados30a59)}</td><td className="font-mono">{number(row.parados60a89)}</td><td className="font-mono text-amber-600 dark:text-amber-400">{number(row.parados90Mais)}</td></tr>)}</tbody></table></div>}</section>
            </div>

            <section className="card p-0 overflow-hidden mb-6"><div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700"><h2 className="section-heading"><PackageSearch size={19} className="text-orange-500" />Capital parado por origem</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Faixas exclusivas mostram onde o capital envelhecido está concentrado.</p></div>{!estoque.porOrigem.length ? <EmptyPanel message="Não há estoque ativo para analisar." /> : <div className="table-container" aria-label="Deslize a tabela para ver mais colunas"><table className="min-w-[700px]"><thead><tr><th scope="col">Origem</th><th scope="col">Unidades</th><th scope="col">Capital</th><th scope="col">30–59 dias</th><th scope="col">60–89 dias</th><th scope="col">90+ dias</th></tr></thead><tbody>{estoque.porOrigem.map((row) => <tr key={row.nome}><td className="font-medium capitalize">{row.nome}</td><td className="font-mono">{number(row.unidades)}</td><td className="font-mono">{money(row.capital)}</td><td className="font-mono">{number(row.parados30a59)}</td><td className="font-mono">{number(row.parados60a89)}</td><td className="font-mono text-amber-600 dark:text-amber-400">{number(row.parados90Mais)}</td></tr>)}</tbody></table></div>}</section>
