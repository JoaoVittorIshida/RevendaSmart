...
const Estoque = () => {
    ...
    return <div className="container">
        ...
        <div className="flex gap-2 overflow-x-auto pb-3 mb-2">{[['todos', 'Todos'], ['disponiveis', 'Disponíveis'], ['reservados', 'Reservados'], ['30+', '30+ dias'], ['60+', '60+ dias'], ['90+', '90+ dias']].map(([value, label]) => <button key={value} onClick={() => setFilter(value)} className={`badge min-h-11 whitespace-nowrap px-3 py-2 ${filter === value ? 'badge-blue' : 'badge-gray'}`}>{label}</button>)}</div>

        <div className="space-y-3">{groups.map((product) => <div key={product.id} className="card p-0 overflow-hidden">
            <button onClick={() => setExpanded(expanded === product.id ? null : product.id)} className="flex w-full flex-col items-stretch justify-between gap-3 p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800 min-[420px]:flex-row min-[420px]:items-center sm:p-5">
                <div className="flex min-w-0 items-center gap-3">{product.foto ? <img src={product.foto} alt="" className="h-12 w-12 shrink-0 rounded-xl object-cover" /> : <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-slate-100 dark:bg-slate-700"><Package /></div>}<div className="min-w-0"><p className="break-words font-bold">{product.nome}</p><p className="break-words text-sm text-slate-500">{product.marca} · {plural(product.itens.length, 'unidade', 'unidades')}</p></div></div>
                <span className="badge badge-blue self-start min-[420px]:shrink-0 min-[420px]:self-auto">{money(product.itens.reduce((sum, item) => sum + item.precoCusto, 0))}</span>
            </button>
            {expanded === product.id && <div className="border-t border-slate-100 dark:border-slate-700 p-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{product.itens.map((item, index) => {
                ...
                return <div key={item.id} className={`rounded-xl border p-4 ${expired ? 'border-amber-300 bg-amber-50/40 dark:border-amber-900 dark:bg-amber-950/10' : 'border-slate-200 dark:border-slate-700'} ${isReservationOpen ? 'sm:col-span-2 lg:col-span-3' : ''}`}>
                    ...
                    <div className="mt-4 flex gap-2">{item.status === 'disponivel' ? <button className="btn btn-secondary flex-1 px-3 py-2 text-xs sm:flex-none" onClick={() => startReservation(item)}><CalendarClock size={15} />Reservar</button> : <button className="btn btn-secondary flex-1 px-3 py-2 text-xs sm:flex-none" onClick={() => release(item)}><Unlock size={15} />Liberar</button>}<button className="grid min-h-11 min-w-11 place-items-center rounded-lg text-red-500 hover:bg-red-50" onClick={() => remove(item)} title="Excluir unidade"><Trash2 size={16} /></button></div>
                    {isReservationOpen && <form onSubmit={reserve} className="mt-4 grid items-end gap-3 border-t border-violet-100 pt-4 dark:border-violet-900 md:grid-cols-[1fr_1fr_auto]"><div><label className="label">Reservar até</label><input required type="date" min={new Date().toLocaleDateString('en-CA')} className="input" value={reserveUntil} onChange={(event) => setReserveUntil(event.target.value)} /></div><div><label className="label">Observação opcional</label><input className="input" maxLength="280" value={observation} onChange={(event) => setObservation(event.target.value)} placeholder="Ex.: confirmado até amanhã" /></div><div className="flex flex-col-reverse gap-2 min-[380px]:flex-row"><button className="btn btn-primary flex-1">Reservar</button><button type="button" className="btn btn-secondary flex-1" onClick={() => setReservation(null)}>Cancelar</button></div></form>}
                </div>;
            })}</div>}
        </div>)}</div>
