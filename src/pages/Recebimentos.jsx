...
const Recebimentos = () => {
    ...
    return <div className="container">
        ...
        <section className="card p-0 overflow-hidden"><div className="flex flex-col gap-4 border-b border-slate-100 px-4 py-4 dark:border-slate-700 md:flex-row md:items-center md:justify-between sm:px-5"><div className="grid grid-cols-2 gap-1 rounded-xl bg-slate-100 p-1 dark:bg-slate-800" role="tablist" aria-label="Situação dos recebimentos"><button type="button" role="tab" aria-selected={tab === 'pendentes'} onClick={() => setTab('pendentes')} className={`min-h-10 rounded-lg px-2 py-2 text-sm font-semibold sm:px-3 ${tab === 'pendentes' ? 'bg-white text-amber-700 shadow-sm dark:bg-slate-700 dark:text-amber-300' : 'text-slate-500 dark:text-slate-400'}`}>Pendentes ({pending.length})</button><button type="button" role="tab" aria-selected={tab === 'historico'} onClick={() => setTab('historico')} className={`min-h-10 rounded-lg px-2 py-2 text-sm font-semibold sm:px-3 ${tab === 'historico' ? 'bg-white text-emerald-700 shadow-sm dark:bg-slate-700 dark:text-emerald-300' : 'text-slate-500 dark:text-slate-400'}`}>Histórico</button></div></div><div className="relative m-4 max-w-sm sm:m-5"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="search" className="input pl-9" placeholder="Buscar produto ou canal..." value={search} onChange={(event) => setSearch(event.target.value)} /></div><div className="table-container">{!filteredRows.length ? <div className="p-8 text-center text-slate-500 sm:p-12"><CircleDollarSign className="mx-auto mb-3 opacity-40" size={30} />{tab === 'pendentes' ? 'Nenhum recebimento pendente.' : 'Nenhum recebimento no histórico.'}</div> : <table className="min-w-[760px]"><thead><tr><th>Venda</th><th>Produto</th><th>Canal</th><th className="text-right">Valor líquido</th><th>{tab === 'pendentes' ? 'Status' : 'Recebido em'}</th>{tab === 'pendentes' && <th className="text-right">Ação</th>}</tr></thead><tbody>{filteredRows.map((sale) => <tr key={sale.id}><td className="font-medium">{formatDate(sale.data)}</td><td><p className="font-semibold">{sale.produto}</p><p className="text-xs text-slate-500">{sale.categoria}</p></td><td>{sale.canal}</td><td className="text-right font-mono font-bold">{money(sale.valor)}</td><td>{tab === 'pendentes' ? <span className="badge badge-yellow">Pendente</span> : <span className="inline-flex items-center gap-1 text-sm font-medium text-emerald-600 dark:text-emerald-400"><CheckCircle2 size={16} />{formatDate(sale.dataRecebimento)}</span>}</td>{tab === 'pendentes' && <td className="text-right"><button type="button" onClick={() => openReceiptModal(sale)} className="btn btn-success px-3 py-2 text-xs">Dar baixa</button></td>}</tr>)}</tbody></table>}</div></section>
