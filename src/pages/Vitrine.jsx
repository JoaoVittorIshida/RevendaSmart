    ...
    return (
        <div className="container">
            <div className="page-header">
                ...
                <div className="grid w-full gap-2 min-[460px]:grid-cols-2 sm:flex sm:w-auto sm:flex-wrap">
                    <button type="button" className="btn btn-secondary w-full sm:w-auto" onClick={openContact}><Store size={17} />Configurações da Loja</button>
                    <button type="button" className="btn btn-primary w-full sm:w-auto" onClick={openCreate} disabled={!availableProducts.length}><Plus size={18} />Adicionar produto</button>
                </div>
            </div>
                ...
                <div className="flex flex-col gap-5 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
                    ...
                    <div className="flex flex-col gap-3 sm:items-end">
                        <div className="flex items-center gap-3">
                            ...
                            <button type="button" role="switch" aria-checked={vitrineConfig.publicada} aria-label="Publicar Vitrine" disabled={changingPublication} onClick={togglePublication} className="relative h-11 w-14 shrink-0 rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 disabled:opacity-60 dark:focus-visible:ring-offset-slate-800">
                                <span className={`pointer-events-none absolute inset-x-0 top-1/2 h-7 -translate-y-1/2 rounded-full transition-colors ${vitrineConfig.publicada ? 'bg-blue-600' : 'bg-slate-300 dark:bg-slate-600'}`} />
                                <span className={`pointer-events-none absolute left-1 top-1/2 h-5 w-5 -translate-y-1/2 rounded-full bg-white shadow-sm transition-transform ${vitrineConfig.publicada ? 'translate-x-6' : 'translate-x-0'}`} />
                            </button>
                        </div>
            ...
            <Modal isOpen={Boolean(adModal)} onClose={() => !savingAd && setAdModal(null)} title={adModal?.mode === 'edit' ? 'Editar anúncio' : 'Adicionar à Vitrine'} maxWidth="max-w-3xl">
                <form onSubmit={submitAd} className="space-y-5">
                    ...
                    <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-5 dark:border-slate-700 sm:flex-row sm:justify-end sm:gap-3"><button type="button" className="btn btn-secondary" onClick={() => setAdModal(null)} disabled={savingAd}>Cancelar</button><button type="submit" className="btn btn-primary" disabled={savingAd}>{savingAd ? 'Salvando...' : adModal?.mode === 'edit' ? 'Salvar alterações' : 'Adicionar à Vitrine'}</button></div>
                </form>
            </Modal>
            ...
            <Modal isOpen={contactOpen} onClose={() => !savingContact && setContactOpen(false)} title="Configurações da Loja" maxWidth="max-w-lg">
                <form onSubmit={submitContact} className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-[1fr_110px]"><div><label className="label" htmlFor="cidade-vitrine">Cidade</label><div className="relative"><MapPin size={17} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input id="cidade-vitrine" className="input pl-10" maxLength="100" value={contact.cidade} onChange={(event) => setContact({ ...contact, cidade: event.target.value })} placeholder="Sua cidade" /></div></div><div><label className="label" htmlFor="estado-vitrine">Estado</label><select id="estado-vitrine" className="select" value={contact.estado} onChange={(event) => setContact({ ...contact, estado: event.target.value })}><option value="">UF</option>{states.map((state) => <option key={state} value={state}>{state}</option>)}</select></div></div>
                    <p className="text-xs text-slate-500">Cidade e estado são opcionais, mas devem ser informados juntos.</p>
                    <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-5 dark:border-slate-700 sm:flex-row sm:justify-end sm:gap-3"><button type="button" className="btn btn-secondary" onClick={() => setContactOpen(false)} disabled={savingContact}>Cancelar</button><button type="submit" className="btn btn-primary" disabled={savingContact}>{savingContact ? 'Salvando...' : 'Salvar configurações'}</button></div>
                </form>
            </Modal>
        </div>
    );
